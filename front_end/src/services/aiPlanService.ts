...
// ——Tool: Return the given date to the Monday of the week (local time zone)——
...
//——Tool: Calculate the week offset difference (integer) of the "Monday" where two dates are located——
...
export function mapAiPlanToWeeklyPlan(aiPlan: any): WeeklyPlan {
  ...
  for (const day of aiPlan.days) {
    ...
    for (const b of (day.blocks || [])) {
      ...
      console.log(`🔍 [mapAiPlanToWeeklyPlan] 任务: ${meta.taskTitle}, Part title: ${b.title}`);
      console.log(`🎯 [GEMINI_TITLE_CHECK] This is a specific title generated by Gemini: "${b.title}"`);
...
/**
*One step approach: Request backend AI plan → Map to WeeklyPlan and return
...
export async function fetchAndMapAiPlan(): Promise<WeeklyPlan> {
  ...
  console.log('🔍 fetchAndMapAiPlan data received:', aiPlan);
  console.log('🔍 Days data of AI plan:', aiPlan?.days);
  console.log('🔍 AISummary data of AI plan:', aiPlan?.aiSummary);
  ...
  if (aiPlan.saved) {
    console.log('✅ [fetchAndMapAiPlan] Backend saved plan, plan ID:', aiPlan.plan_id);
    console.log('🤖 [fetchAndMapAiPlan] The detailed content of AI has been included in the response');
  } else {
    console.log('⚠️ [fetchAndMapAiPlan] Backend plan not saved, using old logic');
    ...
    if (!saveResult.ok) {
      console.error('❌ Learning plan save failed:', saveResult.error);
      throw new Error(`Learning plan save failed: ${saveResult.error || 'unknown err'}`);
    }
    
    console.log('✅ The learning plan has been saved to the database through front-end logic');
  }
  
  //Map AI plan to weekly plan format for front-end display
  const weeklyPlan = mapAiPlanToWeeklyPlan(planData);
  console.log('🗓️ Mapped weekly plan:', weeklyPlan);
